...
- `SMTP_USER` 與 `SMTP_PASS` (用於發送驗證碼)

## Email SMTP 設定 (發送驗證碼)

系統在建立學生帳號時會發送驗證碼。若未設定 SMTP，系統會進入「模擬模式」，並將驗證碼印在終端機畫面上。

**若要使用 Gmail 發信：**
1. 前往 [Google 帳戶的安全性設定](https://myaccount.google.com/security)。
2. 確保已經開啟了**兩步驟驗證**。
3. 搜尋「**應用程式密碼 (App passwords)**」。
4. 應用程式選擇「郵件」並產生密碼。
5. 將獲得的 16 位數密碼加入 `.env` 檔案中：
...
本專案將會把 Ollama 整合進 Docker Compose 中進行共同管理。在運行之前，請確保您的主機具備執行 AI 模型的資源。

針對本專案的「智慧上架助手」與「自動內容審核」功能，建議在 Ollama 中拉取（pull）以下模型：

1. **圖片識別（視覺多模態）模型**：
   - `llava` (預設推薦)：大約需要 8GB VRAM。效能與辨識能力佳，適合用來判斷商品圖片是否包含違禁品。
   - `moondream`：若主機沒有獨立顯卡或資源較小，可改用此輕量級多模態模型。

2. **純文字與意圖解析模型**（針對繁體中文優化）：
   - `qwen2` 或 `qwen2.5`：中文理解能力極佳，適合做商品描述潤飾。
   - `llama3` 或 `llama3.1`：通用性廣，也可以勝任絕大多數文字處理任務。
