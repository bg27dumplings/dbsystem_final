...
import { countUnreadChatMessages, countUnreadAppointments, findPendingReviews } from "@/lib/marketplace/queries";
...
export async function ResponsiveShell({ children }: { children: React.ReactNode }) {
  ...
  const unreadChatCount = session ? await countUnreadChatMessages(session.studentId) : 0;
  const unreadAppointmentCount = session ? await countUnreadAppointments(session.studentId) : 0;
          ...
          <div className="hidden items-center gap-3 md:flex">
            <nav aria-label="主要導覽" className="flex items-center gap-1">
              {navItems.map((item) => {
                ...
                const badgeCount = item.href === "/me" ? unreadChatCount + unreadAppointmentCount : 0;
